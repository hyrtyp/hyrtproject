...
public class FoshanLawActivity extends Activity {
								...
								try {
									...
									if (!IS_DEBUG
										...
										if (result.contains(DEVICE_TYPE)
												&& result
														.contains(getDeviceUniqueIdentifier())) {
											// 设备号匹配
									...
									if (TimeOutHelper.ALDATA_FLAG
										...
										GlobalFunction
												.init_NotiSetting(FoshanLawActivity.this);
										// 启动服务 by zhengwei
										PPTService
												.Start(FoshanLawActivity.this);
	...
	protected String getDeviceUniqueIdentifier() {
		if (deviceUniqueIdentifier == null)
			deviceUniqueIdentifier = ((TelephonyManager) this
					.getSystemService(Context.TELEPHONY_SERVICE)).getDeviceId();
		if (deviceUniqueIdentifier == null
				|| deviceUniqueIdentifier.length() == 0)
			deviceUniqueIdentifier = android.provider.Settings.Secure
	...
	private Thread updateThread = new Thread() {
		public void run() {
			try {
				URL url = new URL(
						"http://192.168.1.11:8090/LCSISTerminal/version");
				...
				if (Integer.parseInt(getStreamString(is)) > getVersionCode(FoshanLawActivity.this)) {
					...
					String apkUrl = "http://192.168.1.11:8090/LCSISTerminal/foshan.apk";
