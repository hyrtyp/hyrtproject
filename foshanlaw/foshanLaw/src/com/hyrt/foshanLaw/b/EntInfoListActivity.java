		...
		public void dispatchMessage(Message msg) {
			for (int i = 0; i < dataEntInfos.length(); i++) {
				...
				if (commonListAdapter == null) {
					commonListAdapter = new CommonListAdapter(EntInfoListActivity.this, entInfos, msg.what);
		...
		try {
			...
			if (!getIntent().getBooleanExtra("point", false)) {
				...
				if ("null".equals(entInfos
					...
					intent.putExtra(ArgGisActivity.GIS_DATA_KEY, entInfos
							.getJSONObject(position).getString("LAT")
							+ "!@"
			...
			public void run() {
				...
				try {
					...
					if (level.equals("jz")) {
						...
						mwpmBussEntinfo.setUserid(null);
						mwpmBussEntinfo.setReseauid(settings.getString("reseauid", ""));
					} else {
						mwpmBussEntinfo.setUserid(settings.getString("userid",
								""));
						mwpmBussEntinfo.setReseauid(null);
