...
public class EntInfoListActivity extends Activity implements OnClickListener,
	...
	private Handler dataHandler = new Handler() {
		...
		public void dispatchMessage(Message msg) {
			for (int i = 0; i < dataEntInfos.length(); i++) {
				try {
					boolean isAdd = true;
					for (int j = 0; j < entInfos.length(); j++) {
						try {
							if (entInfos
									.getJSONObject(j)
									.getString("NAME")
									.equals(dataEntInfos.getJSONObject(i)
											.getString("NAME"))) {
								isAdd = false;
							}
						} catch (Exception ex) {
		...
		new Thread() {
			...
			public void run() {
				...
				try {
					...
					SharedPreferences settings = getSharedPreferences(
							"userinfo", Activity.MODE_PRIVATE);
					String reseauid = settings.getString("reseauid", "");
			...
			public void onClick(DialogInterface dialog, int which) {
				try {
					...
					if (lat == null || long1 == null || lat.equals("")
						...
						builder.create().show();
					} else if (myLatLon == null || myLatLon[0] == null) {
						AlertDialog.Builder builder = new Builder(
								EntInfoListActivity.this);
						builder.setTitle("提示");
						...
						Intent intent = new Intent(EntInfoListActivity.this,
								GDMapActivity.class);
						intent.putExtra(GDMapActivity.GIS_KEY,
								GDMapActivity.DRAW_LINE_FLAG);
						intent.putExtra(GDMapActivity.GIS_DATA_KEY, lat + "!@"
								+ long1);
						startActivity(intent);
					}
				} catch (JSONException e) {
	...
	private void clickCommonUserListItem(final int position) {
		try {
			...
			if (!getIntent().getBooleanExtra("point", false)) {
				...
				intent.putExtra(GDMapActivity.GIS_KEY,
						GDMapActivity.DRAW_POINT_FLAG);
				intent.putExtra(GDMapActivity.GIS_DATA_KEY, entInfos
						.getJSONObject(position).getString("LAT")
						+ "!@"
						+ entInfos.getJSONObject(position).getString("LONG1"));
				startActivity(intent);
			}
		} catch (JSONException e) {
			// TODO Auto-generated catch block
			...
			public void run() {
				...
				try {
					...
					String jsonResult = Service.queryCompanyByPatrolId(
							getIntent().getStringExtra("id"), pageNo, reseauid);
					String errorCode = TimeOutHelper.ALDATA_FLAG;
					try {
						dataEntInfos = new JSONArray(jsonResult);
					} catch (Exception e) {
	...
	private void clickEntInfoItem(final int position) {
		...
		if (myLatLon == null || myLatLon[0] == null) {
			AlertDialog.Builder builder = new Builder(EntInfoListActivity.this);
			builder.setTitle("提示");
			builder.setMessage("获取不到当前位置，无法标注！");
			builder.create().show();
		} else {
			AlertDialog.Builder builder = new Builder(this);
			builder.setTitle("提示");
			...
			builder.setPositiveButton("确认",
					new DialogInterface.OnClickListener() {
						...
						@Override
						public void onClick(DialogInterface dialog, int which) {
							gisEntInfos(position, MyMapActivity.latLon);
							dialog.dismiss();
						}
					});
